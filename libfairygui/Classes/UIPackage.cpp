...
bool UIPackage::loadPackage(ByteBuffer* buffer)
{
    ...
    for (int i = 0; i < cnt; i++)
    {
        ...
        sprite->atlas = pi;
            sprite->rect.origin.x = buffer->readInt() * CC_CONTENT_SCALE_FACTOR();
            sprite->rect.origin.y = buffer->readInt() * CC_CONTENT_SCALE_FACTOR();
            sprite->rect.size.width = buffer->readInt() * CC_CONTENT_SCALE_FACTOR();
            sprite->rect.size.height = buffer->readInt() * CC_CONTENT_SCALE_FACTOR();
        sprite->rotated = buffer->readBool();
        if (ver2 && buffer->readBool())
        {
                sprite->offset.x = buffer->readInt() * CC_CONTENT_SCALE_FACTOR();
                sprite->offset.y = buffer->readInt() * CC_CONTENT_SCALE_FACTOR();
                sprite->originalSize.width = buffer->readInt() * CC_CONTENT_SCALE_FACTOR();
                sprite->originalSize.height = buffer->readInt() * CC_CONTENT_SCALE_FACTOR();
        }
        else
        {
...
void UIPackage::loadFont(PackageItem* item)
{
    ...
    int fontSize = buffer->readInt() * CC_CONTENT_SCALE_FACTOR();
    int xadvance = buffer->readInt() * CC_CONTENT_SCALE_FACTOR();
    int lineHeight = buffer->readInt() * CC_CONTENT_SCALE_FACTOR();
    ...
    for (int i = 0; i < cnt; i++)
    {
        ...
        bx = buffer->readInt() * CC_CONTENT_SCALE_FACTOR();
        by = buffer->readInt() * CC_CONTENT_SCALE_FACTOR();
        def.offsetX = buffer->readInt() * CC_CONTENT_SCALE_FACTOR();
        def.offsetY = buffer->readInt() * CC_CONTENT_SCALE_FACTOR();
        bw = buffer->readInt() * CC_CONTENT_SCALE_FACTOR();
        bh = buffer->readInt() * CC_CONTENT_SCALE_FACTOR();
        def.xAdvance = buffer->readInt() * CC_CONTENT_SCALE_FACTOR();
