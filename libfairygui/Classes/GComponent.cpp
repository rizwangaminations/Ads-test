...
void GComponent::setMask(cocos2d::Node* value, bool inverted, bool swallowTouches)
{
    if (_maskOwner)
    {
    ...
    if (value)
    {
        for (auto& child : _children)
        {
            if (child->_displayObject == value)
            {
                ...
                _maskOwner->handlePositionChanged();
                _maskOwner->retain();
                _maskSwallowTouches = swallowTouches;
                break;
            }
        }
    }
...
bool GComponent::isMaskSwallowTouches()
{
    return _maskSwallowTouches;
}

void GComponent::setMaskSwallowTouches(bool swallowTouches)
{
    _maskSwallowTouches = swallowTouches;
}
...
GObject* GComponent::hitTest(const Vec2& worldPoint, const Camera* camera)
{
    ...
    if (_maskOwner && _maskSwallowTouches)
    {
