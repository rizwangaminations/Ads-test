...
std::unordered_map<cocos2d::Node*, GRoot*> GRoot::_instanceMap;
bool GRoot::_soundEnabled = true;
float GRoot::_soundVolumeScale = 1.0f;
int GRoot::contentScaleLevel = 0;

GRoot* GRoot::create(Node* node, int zOrder)
{
    GRoot* pRet = new (std::nothrow) GRoot(node);
    if (pRet && pRet->initWithNode(node, zOrder))
    {
        pRet->autorelease();
        return pRet;
    }
...
GRoot::GRoot(cocos2d::Node* container)
...
{
    _instanceMap[container] = this;
}

GRoot::~GRoot()
{
    _inputProcessor->setCaptureCallback(nullptr);
...
{
    if (GRoot::_instanceMap.empty())
        return nullptr;
    return _instanceMap.begin()->second;
}
...
bool GRoot::initWithNode(cocos2d::Node* node, int zOrder)
{
    if (!GComponent::init())
        return false;
    ...
    node->addChild(_displayObject, zOrder);
